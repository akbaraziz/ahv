...
def esxi_single_vm_run(spec):   
    ...
    vm_ip = "@@{vm_ip}@@"
    bp_name = "bp-@@{calm_time}@@-@@{calm_unique}@@"
    ...
    def get_brownfield_vms_list(base_url, auth, project_uuid, account_uuid, vm_ip):
        ...
        payload = {
            "length":100,
            "offset":0,
            "filter":"project_uuid=={0};account_uuid=={1}".format(project_uuid, account_uuid)
        }
        print("Making a {} API call to {}".format(method, url))
        resp = urlreq(
            url,
        ...
        if resp.ok:
            ...
            if json_resp['metadata']['total_matches'] > 0:
                for vm in json_resp['entities']:
                    if len(set(vm['status']['resources']['address']) & set([vm_ip])) > 0:
                        vm_info = {
                ...
                return brownfield_instance_list
            else:
    ...
    brownfield_instance_list = get_brownfield_vms_list(base_url, auth, project_uuid, account_uuid, vm_ip)
